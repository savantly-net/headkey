...
public class JpaMemorySystemController {
    ...
    private static final Logger LOG = Logger.getLogger(
        JpaMemorySystemController.class
    );
    ...
    public Response healthCheck() {
        ...
        try {
            ...
            healthStatus.put(
                "memorySystem",
                createMemorySystemHealthInfo(memorySystemHealthy)
            );
            ...
            healthStatus.put(
                "database",
                createDatabaseHealthInfo(databaseHealthy)
            );
            ...
            healthStatus.put(
                "similarityStrategy",
                createStrategyHealthInfo(strategyHealthy)
            );

            // Overall health status
            isHealthy =
                memorySystemHealthy && databaseHealthy && strategyHealthy;
    ...
    public Response getConfiguration() {
        try {
            ...
            databaseConfig.put(
                "autoCreateSchema",
                properties.database().autoCreateSchema()
            );
            ...
            poolConfig.put(
                "timeoutMs",
                properties.database().pool().timeoutMs()
            );
            ...
            performanceConfig.put(
                "enableStatistics",
                properties.performance().enableStatistics()
            );
            performanceConfig.put(
                "cacheSize",
                properties.performance().cacheSize()
            );
            performanceConfig.put(
                "enableAsync",
                properties.performance().enableAsync()
            );
            ...
            runtime.put(
                ...
                memorySystem
                    .getSimilaritySearchStrategy()
                    .supportsVectorSearch()
            );
            runtime.put("actualBatchSize", memorySystem.getBatchSize());
            runtime.put(
                "actualMaxSimilarityResults",
                memorySystem.getMaxSimilaritySearchResults()
            );
            runtime.put(
                "actualSimilarityThreshold",
                memorySystem.getSimilarityThreshold()
            );
            runtime.put(
                "actualSecondLevelCache",
                memorySystem.isSecondLevelCacheEnabled()
            );
            ...
            error.put(
                "error",
                "Failed to retrieve configuration: " + e.getMessage()
            );
    ...
    public Response getDatabaseCapabilities() {
        try {
            ...
            var dbCapabilities =
                JpaSimilaritySearchStrategyFactory.analyzeDatabaseCapabilities(
                    entityManager
                );

            capabilities.put(
                "databaseType",
                dbCapabilities.getDatabaseType().toString()
            );
            capabilities.put("version", dbCapabilities.getVersion());
            capabilities.put(
                "hasVectorSupport",
                dbCapabilities.hasVectorSupport()
            );
            capabilities.put(
                "hasFullTextSupport",
                dbCapabilities.hasFullTextSupport()
            );

            // Get strategy recommendation
            var recommendation = JpaMemorySystemFactory.analyzeDatabase(
                entityManager.getEntityManagerFactory()
            );
            capabilities.put(
                "recommendedStrategy",
                recommendation.getRecommendedStrategy().getStrategyName()
            );
            capabilities.put(
                "currentStrategy",
                memorySystem.getSimilaritySearchStrategy().getStrategyName()
            ...
            LOG.errorf(
                e,
                "Error analyzing database capabilities: %s",
                e.getMessage()
            );
            Map<String, Object> error = new HashMap<>();
            error.put(
                "error",
                "Failed to analyze database capabilities: " + e.getMessage()
            );
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                .entity(error)
                .build();
    ...
    public Response getStatistics() {
        try {
            ...
            Map<String, Object> beliefStats =
                beliefStorageService.getStorageStatistics();
            ...
            if (entityManager.getEntityManagerFactory().isOpen()) {
                ...
                dbInfo.put(
                    "configuredDatabaseKind",
                    properties.database().kind()
                );
    ...
    private Map<String, Object> createMemorySystemHealthInfo(boolean healthy) {
        ...
        info.put(
            "supportsVectorSearch",
            memorySystem.getSimilaritySearchStrategy().supportsVectorSearch()
        );
        return info;
    }
