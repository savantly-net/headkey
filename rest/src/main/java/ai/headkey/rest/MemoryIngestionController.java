...
@Path("/api/v1/memory")
@ApplicationScoped
@Tag(name = "Memory Ingestion", description = "Operations for ingesting and managing memories")
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class MemoryIngestionController {
    
    private static final Logger LOG = Logger.getLogger(MemoryIngestionController.class);
    ...
    public MemoryIngestionController() {
        LOG.info("MemoryIngestionController instantiated - dependencies will be injected by CDI");
    }
    ...
    @APIResponses({
        @APIResponse(
            ...
            content = @Content(schema = @Schema(implementation = MemoryIngestionResponse.class))
        ),
        @APIResponse(
            responseCode = "400",
    ...
    public Response ingestMemory(@Valid MemoryIngestionRequest request) {
        ...
        try {
            ...
            if (!validationErrors.isEmpty()) {
                LOG.warnf("Validation failed for request: %s", validationErrors);
                MemoryIngestionResponse errorResponse = mapper.createValidationErrorResponse(validationErrors);
            ...
            if (request.isDryRunEffective()) {
                ...
                LOG.infof("Dry run completed for agent: %s", request.getAgentId());
            } else {
                result = ingestionModule.ingest(memoryInput);
                LOG.infof("Memory ingested successfully with ID: %s", result.getMemoryId());
            }
            ...
            MemoryIngestionResponse response = mapper.toMemoryIngestionResponse(result);
            ...
            LOG.errorf(e, "Invalid input for memory ingestion: %s", e.getMessage());
            MemoryIngestionResponse errorResponse = mapper.createErrorResponse(
                "Input validation failed: " + e.getMessage(), e);
            return Response.status(Response.Status.BAD_REQUEST).entity(errorResponse).build();
            
        } catch (IllegalArgumentException e) {
            LOG.errorf(e, "Illegal argument in memory ingestion: %s", e.getMessage());
            MemoryIngestionResponse errorResponse = mapper.createErrorResponse(
                "Invalid request: " + e.getMessage(), e);
            return Response.status(Response.Status.BAD_REQUEST).entity(errorResponse).build();
            
        } catch (Exception e) {
            LOG.errorf(e, "Unexpected error during memory ingestion: %s", e.getMessage());
            MemoryIngestionResponse errorResponse = mapper.createErrorResponse(
                "Internal server error occurred during ingestion", e);
    ...
    public Response dryRunIngest(@Valid MemoryIngestionRequest request) {
        ...
        try {
            ...
            List<String> validationErrors = mapper.validateRequest(request);
            if (!validationErrors.isEmpty()) {
                LOG.warnf("Validation failed for dry run: %s", validationErrors);
                MemoryIngestionResponse errorResponse = mapper.createValidationErrorResponse(validationErrors);
                return Response.status(Response.Status.BAD_REQUEST).entity(errorResponse).build();
            }
            ...
            LOG.infof("Dry run completed successfully for agent: %s", request.getAgentId());
            ...
            LOG.errorf(e, "Unexpected error during dry run: %s", e.getMessage());
            MemoryIngestionResponse errorResponse = mapper.createErrorResponse(
                "Internal server error occurred during dry run", e);
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR).entity(errorResponse).build();
        }
    }
    ...
    public Response getStatistics() {
        try {
            Map<String, Object> statistics = ingestionModule.getIngestionStatistics();
    ...
    public Response healthCheck() {
        try {
            ...
            if (healthy) {
                ...
                healthStatus.put("message", "One or more dependencies are unhealthy");
    ...
    public Response validateInput(@Valid MemoryIngestionRequest request) {
        try {
            ...
            if (validationErrors.isEmpty()) {
                ...
                LOG.debugf("Input validation passed for agent: %s", request.getAgentId());
