...
public class MemorySystemConfig {
    ...
    public JpaMemoryEncodingSystem jpaMemoryEncodingSystem() {
        ...
        try {
            ...
            JpaSimilaritySearchStrategy strategy = createSimilaritySearchStrategy();
            ...
            JpaMemoryEncodingSystem memorySystem = JpaMemorySystemFactory.builder()
                ...
                .maxSimilaritySearchResults(properties.maxSimilarityResults())
                .similarityThreshold(properties.similarityThreshold())
                .build();
            
            LOG.infof("JpaMemoryEncodingSystem successfully initialized with strategy: %s", 
                     strategy.getStrategyName());
            return memorySystem;
            
        } catch (Exception e) {
            LOG.errorf(e, "Failed to initialize JpaMemoryEncodingSystem: %s", e.getMessage());
            throw new RuntimeException("Unable to initialize JPA memory system", e);
        }
    }
     ...
     * 
     * For now, this creates a simple implementation that delegates to the
     * MemoryEncodingSystem. In a full implementation, this would be a
     * separate component with additional ingestion logic.
     * 
    ...
    @Produces
    @Singleton
    public InformationIngestionModule informationIngestionModule(JpaMemoryEncodingSystem memorySystem) {
        LOG.info("Creating InformationIngestionModule based on JpaMemoryEncodingSystem");
        
        // For this refactoring, we'll create a simple adapter
        // In a real implementation, this would be a proper ingestion module
        return new JpaInformationIngestionAdapter(memorySystem);
    }
    ...
    private JpaSimilaritySearchStrategy createSimilaritySearchStrategy() {
        ...
        switch (strategyName.toLowerCase()) {
            case "auto":
                ...
                return JpaSimilaritySearchStrategyFactory.createStrategy(entityManager);
            ...
            default:
                LOG.warnf("Unknown strategy '%s', falling back to auto-detection", strategyName);
                return JpaSimilaritySearchStrategyFactory.createStrategy(entityManager);
        }
    }
    ...
    private AbstractMemoryEncodingSystem.VectorEmbeddingGenerator createLangChain4JEmbeddingGenerator() {
        ...
        LOG.infof("Creating LangChain4J embedding generator with model: %s",
    ...
    private AbstractMemoryEncodingSystem.VectorEmbeddingGenerator createFallbackEmbeddingGenerator() {
        LOG.warn("Using fallback mock embedding generator - not suitable for production");
