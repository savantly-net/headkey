...
public class DefaultJpaSimilaritySearchStrategy implements JpaSimilaritySearchStrategy {
    ...
    public String getStrategyName() {
        ...
        return "Default JPA Similarity Search (delegating to: " + delegateStrategy.getStrategyName() + ")";
    }
     ...
     */
    private void ensureInitialized(EntityManager entityManager) throws Exception {
        if (!isInitialized) {
            initializeDelegate(entityManager);
        }
    }
     ...
     * @throws Exception if initialization fails
     */
    private void initializeDelegate(EntityManager entityManager) throws Exception {
        try {
            databaseType = detectDatabaseType(entityManager);
            
            switch (databaseType) {
                case POSTGRESQL:
                    delegateStrategy = new PostgresJpaSimilaritySearchStrategy();
                    break;
            ...
            System.err.println("Warning: Failed to detect database type or initialize preferred strategy. " +
                             "Falling back to TextBasedJpaSimilaritySearchStrategy. Error: " + e.getMessage());
        }
    }
     ...
     */
    private DatabaseType detectDatabaseType(EntityManager entityManager) throws Exception {
        try {
            // Check Hibernate dialect configuration
            Object dialectProperty = entityManager.getEntityManagerFactory()
            ...
            try {
                ...
                entityManager.createNativeQuery("SELECT version()").setMaxResults(1).getSingleResult();
                Object result = entityManager.createNativeQuery("SELECT version()").setMaxResults(1).getSingleResult();
            ...
            try {
                // H2 detection query
                entityManager.createNativeQuery("SELECT H2VERSION()").setMaxResults(1).getSingleResult();
                return DatabaseType.H2;
            } catch (Exception e) {
