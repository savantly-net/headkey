...
public class JpaMemoryEncodingSystem extends AbstractMemoryEncodingSystem {
     ...
     */
    public JpaMemoryEncodingSystem(
    ...
    public JpaMemoryEncodingSystem(
        ...
        this.maxSimilaritySearchResults = Math.max(
            1,
            maxSimilaritySearchResults
        );
        ...
        if (similaritySearchStrategy != null) {
            ...
            try {
                this.similaritySearchStrategy =
                    JpaSimilaritySearchStrategyFactory.createStrategy(em);
                this.similaritySearchStrategy.initialize(em);
            } catch (Exception e) {
                throw new StorageException(
                    "Failed to initialize similarity search strategy",
                    e
                );
            } finally {
                em.close();
    ...
    protected MemoryRecord doEncodeAndStore(
        ...
        try {
            ...
            System.err.println(
                "  Memory ID: " + (entity != null ? entity.getId() : "null")
            );
            System.err.println(
                "  Agent ID: " + (entity != null ? entity.getAgentId() : "null")
            );
            System.err.println(
                "  Content length: " +
        ...
        try {
            ...
            for (int i = 0; i < idList.size(); i += batchSize) {
                ...
                TypedQuery<MemoryEntity> query = em.createQuery(
                    "SELECT m FROM MemoryEntity m WHERE m.id IN :ids",
                    MemoryEntity.class
                );
    ...
    protected MemoryRecord doUpdateMemory(
        ...
        try {
            ...
            MemoryEntity entity = em.find(
                MemoryEntity.class,
                memoryRecord.getId()
            );
            if (entity == null) {
                throw new MemoryNotFoundException(
                    "Memory not found: " + memoryRecord.getId()
                );
            }
        ...
        try {
            ...
            if (limit > 0) {
                query = em.createNamedQuery(
                    "MemoryEntity.findByAgentIdAndLimit",
                    MemoryEntity.class
                );
                query.setMaxResults(limit);
            } else {
                query = em.createNamedQuery(
                    "MemoryEntity.findByAgentId",
                    MemoryEntity.class
                );
            }
            ...
            List<MemoryRecord> results = entities
                .stream()
    ...
    public List<MemoryRecord> getMemoriesInCategory(
        ...
        try {
            ...
            CriteriaQuery<MemoryEntity> query = cb.createQuery(
                MemoryEntity.class
            );
    ...
    public List<MemoryRecord> getOldMemories(
        ...
        try {
            ...
            if (agentId != null && !agentId.trim().isEmpty()) {
                query = em.createNamedQuery(
                    "MemoryEntity.findOldMemoriesByAgent",
                    MemoryEntity.class
                );
                query.setParameter("agentId", agentId);
            } else {
                query = em.createNamedQuery(
                    "MemoryEntity.findOldMemories",
                    MemoryEntity.class
                );
            }
    ...
    protected void addImplementationSpecificStatistics(
        ...
        try {
            // Get total count
            TypedQuery<Long> countQuery = em.createNamedQuery(
                "MemoryEntity.countTotal",
                Long.class
            );
            ...
            if (entityManagerFactory.getMetamodel() != null) {
                stats.put(
                    "managedTypes",
                    entityManagerFactory.getMetamodel().getManagedTypes().size()
                );
            }
        } catch (Exception e) {
    ...
    protected boolean doHealthCheck() {
        ...
        try {
            ...
            em
                .createQuery("SELECT COUNT(m) FROM MemoryEntity m")
                .getSingleResult();
    ...
    @Override
    protected void addImplementationSpecificCapacityInfo(
    ...
    private void validateEntityManagerFactory() {
        ...
        if (!entityManagerFactory.isOpen()) {
            throw new IllegalArgumentException(
                "EntityManagerFactory must be open"
            );
        }
    }
    ...
    private void updateLastAccessedInTransaction(
        EntityManager em,
        MemoryEntity entity
    ) {
        EntityTransaction transaction = em.getTransaction();
